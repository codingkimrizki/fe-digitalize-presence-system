<template>
  <a-flex justify="flex-start" wrap="wrap" gap="small" class="mb-large">
    <a-space direction="vertical" :size="1">
      ...
      <a-input v-model:value="dashboardStore.filter.search" placeholder="Type machine number or tonnage" allowClear
    ...
    <a-space direction="vertical" :size="1">
      ...
      <a-select class="select-filter" v-model:value="dashboardStore.filter.sortBy">
        ...
        <a-select-option value="targetInjection">Target Injection</a-select-option>
...
watch(
  [() => dashboardStore.filter.search, () => dashboardStore.filter.sortBy, () => dashboardStore.filter.order],
  () => {
    debounceFetch()
  }
)
