...
export const useLogMachineStore = defineStore('logMachine', {
  state: () => ({
    logs: [],
    filter: {
      dateRange: [dayjs().subtract(5, 'days').startOf('day'), dayjs().endOf('day')]
    }
  }),
  actions: {
    async get() {
      try {
        ...
        const response = await api.get(`/machines/${machineDashboardStore.modal.idMachine}/logs`, { params })
        if (response.status === 200) {
          this.logs = response.data.data
        }
      } catch (error) {
        message.error(error.response?.data?.message || 'Error while get log action')
        console.error(error)
      }
    }
  },
