...
export function convertRoutesToMenu(routes) {
  ...
  const menuRoutes = routes.filter(route => route.meta && route.meta.showInMenu === true);
  ...
  return menuRoutes.map((route, index) => {
    ...
    if (route.children && route.children.length > 0) {
      ...
      const childRoutes = route.children.filter(child => child.meta && child.meta.showInMenu !== false);
